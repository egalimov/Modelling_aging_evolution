...
feeding_rates = {1:1, 2:1.1, 3:1.07, 4:0.76, 5:0.69, 6:0.62, 7:0.59, 8:0.33,9:0.16, 10:0.09, 11:0.03, 12:0.03, 13:0.03, 14:0.01, 15:0.01,16:0.01,17:0.01,18:0.01}
for x in range(19,999):
    feeding_rates[x] = 0
        ...
        for a in range(0,pop_length):
            if Organism.population[a].stage != 'dead':
                ...
                if Organism.population[a].stage == 'A':        
                    if d.iloc[fry, frx] > food_consumotion_A*feeding_rates.get(Organism.population[a].adultAge):
                        consumedA = food_consumotion_A*feeding_rates.get(Organism.population[a].adultAge)
                        fc_A = fc_A + consumedA
                    else:
                        consumedA = d.iloc[fry, frx]
                        fc_A = fc_A + consumedA
                    d.iloc[fry, frx] = d.iloc[fry, frx] - food_consumotion_A*feeding_rates.get(Organism.population[a].adultAge)
